...
import { getAllStudents, deleteStudent, sortStudentsBy, searchStudentsBy } from "@/utils/api";
...
export default function StudentManagementPage() {
  ...
  const handleSort = async (column: string) => {
    try {
      if (sortColumn === column) {
        ...
        const response = await sortStudentsBy(column, newOrder);
        setStudents(response.data);
      } else {
        // New column, reset to ASC
        setSortColumn(column);
        setSortOrder("ASC");
        const response = await sortStudentsBy(column, "ASC");
        setStudents(response.data);
      }
    } catch (error) {
  ...
  const handleSearch = async () => {
    try {
      setLoading(true);
      const response = await searchStudentsBy(searchQuery);
      setStudents(response.data.filter((student: Student) => 
  ...
  const handleEdit = (id: number) => {
    router.push(`/student-manage/edit/${id}`);
  };
  ...
  const handleCreate = () => {
    router.push("/student-manage/create");
  };
                  ...
                  {students.length === 0 ? (
                    ...
                    students.map((student) => (
                      <tr key={student.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{student.userFullName}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{student.gender}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{student.nickname}</td>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{student.point}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{student.heart}</td>
